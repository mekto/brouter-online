...
class LocateComponent extends React.Component {
  ...
  toggleLocate() {
    if (this.state.locateStatus === 'off')
      this.locate();
    else
      this.stopLocating();
  }
  ...
  locate() {
    ...
    this.setState({locateStatus: 'searching'});
  }

  stopLocating() {
    this.props.map.stopLocate();
    this.setState({locateStatus: 'off'});
    ...
    return (
      <div className="leaflet-bar">
        <a className="control-button" onClick={this.toggleLocate.bind(this)} title="Show my location">
          <svg width="14" height="14" className={`icon icon-locate icon-locate-${this.state.locateStatus}`} viewBox="0 0 24 24">
